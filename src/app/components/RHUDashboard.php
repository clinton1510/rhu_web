...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_referral') {
        ...
        if ($newReferral['patientName'] === '' || $newReferral['referredTo'] === '' || $newReferral['reason'] === '' || $newReferral['diagnosis'] === '') {
    ...
    if ($action === 'save_opd') {
        $newConsultation = [
            ...
            'barangay' => trim($_POST['barangay'] ?? ''),
            'diagnosis' => trim($_POST['diagnosis'] ?? ''),
            'physician' => trim($_POST['physician'] ?? 'Dr. Santos'),
            'date' => trim($_POST['date'] ?? date('Y-m-d')),
            'disposition' => trim($_POST['disposition'] ?? 'outpatient'),
            'philhealthCharged' => isset($_POST['philhealthCharged']),
            'chiefComplaint' => trim($_POST['chiefComplaint'] ?? ''),
            'icd10' => trim($_POST['icd10'] ?? ''),
            'medications' => array_filter(array_map('trim', explode(',', trim($_POST['medications'] ?? '')))),
        ];
        if ($newConsultation['patientName'] === '' || $newConsultation['diagnosis'] === '') {
            $_SESSION['rhu_error'] = 'Please fill in the consultation patient and diagnosis details.';
        } else {
            $_SESSION['rhu_opd'][] = $newConsultation;
            $_SESSION['rhu_flash'] = 'Consultation record added successfully.';
        }
    ...
    if ($action === 'save_patient') {
        $newPatient = [
            ...
            'bloodType' => trim($_POST['bloodType'] ?? ''),
            'diagnosis' => trim($_POST['diagnosis'] ?? ''),
            'admissionDate' => trim($_POST['admissionDate'] ?? date('Y-m-d')),
            'philhealthCharged' => isset($_POST['philhealthCharged']),
            'philhealthNo' => trim($_POST['philhealthNo'] ?? ''),
            'disposition' => trim($_POST['disposition'] ?? 'admitted'),
        ];
        if ($newPatient['name'] === '' || $newPatient['diagnosis'] === '' || $newPatient['bloodType'] === '') {
            $_SESSION['rhu_error'] = 'Please fill in the new patient details.';
        } else {
            $_SESSION['rhu_patients'][] = $newPatient;
            $_SESSION['rhu_flash'] = 'Patient admitted successfully.';
        }
        header('Location: ' . dashboardUrl('patients'));
        exit;
    }
    if ($action === 'save_transfusion') {
        $newTransfusion = [
            'id' => 'T-' . time(),
            'patientName' => trim($_POST['patientName'] ?? ''),
            'bloodType' => trim($_POST['bloodType'] ?? ''),
            ...
            'date' => trim($_POST['date'] ?? date('Y-m-d')),
            'transfusedBy' => trim($_POST['transfusedBy'] ?? 'Dr. Santos'),
            'component' => trim($_POST['component'] ?? 'Packed RBC'),
            'reaction' => trim($_POST['reaction'] ?? 'None'),
        ];
        if ($newTransfusion['patientName'] === '' || $newTransfusion['bloodType'] === '' || $newTransfusion['units'] < 1) {
            $_SESSION['rhu_error'] = 'Please complete the transfusion record.';
        } else {
            $_SESSION['rhu_transfusions'][] = $newTransfusion;
            $_SESSION['rhu_flash'] = 'Transfusion recorded successfully.';
        }
        header('Location: ' . dashboardUrl('transfusions'));
        exit;
    }
}
            ...
            <div class="flex flex-wrap items-center justify-between gap-2">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-blue-200"><?php echo esc($RHU_INFO['name']); ?> â€” <?php echo esc($RHU_INFO['code']); ?> | <?php echo esc($RHU_INFO['municipality']); ?>, <?php echo esc($RHU_INFO['province']); ?></p>
                    </div>
                </div>
    ...
    <main class="px-4 sm:px-6 py-5 pb-28 sm:pb-5">
            ...
            <div class="space-y-4 sm:space-y-5">
                <div class="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-red-700">Dengue cluster in Halang/Mabini â€¢ <?php echo $criticalMeds + $lowMeds; ?> medicine items need resupply â€¢ <?php echo $highRiskMom; ?> high-risk pregnant patients</p>
                    </div>
                        ...
                        <div class="space-y-2">
                            <?php foreach ($mockOPDConsultations as $c): ?>
                                <div class="rounded-xl border border-gray-100 p-3">
                                    <p class="font-bold text-gray-900"><?php echo esc($c['patientName']); ?> <span class="text-gray-500 text-xs"><?php echo esc($c['age']); ?>y â€¢ <?php echo esc($c['gender']); ?></span></p>
                            ...
                            <div class="p-4">
                                <div class="flex items-start justify-between gap-3">
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500 mt-1"><?php echo esc($c['barangay']); ?> â€¢ <?php echo esc($c['physician']); ?> â€¢ <?php echo esc($c['date']); ?></p>
                                    </div>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <?php foreach ($mockRHUInventory as $inv): ?>
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3"><?php echo $inv['expiringUnits'] > 0 ? esc($inv['expiringUnits']) : 'â€”'; ?></td>
                        ...
                        <div class="bg-white rounded-xl p-4 shadow-sm border-l-4 <?php echo $req['urgency'] === 'critical' ? 'border-red-500' : ($req['urgency'] === 'urgent' ? 'border-orange-400' : 'border-yellow-400'); ?>">
                            <div class="flex flex-wrap items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 mt-0.5">Blood Type: <strong class="text-red-600"><?php echo esc($req['bloodType']); ?></strong> â€¢ <?php echo esc($req['quantity']); ?> unit(s)</p>
                                </div>
                        ...
                        <div class="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-gray-100">
                            ...
                            <div class="mt-3 pt-3 border-t border-gray-100">
                                ...
                                <div class="flex items-center justify-between mt-2">
                                    <p class="text-xs text-gray-500">By: <?php echo esc($ref['referringMD']); ?> â€¢ <?php echo esc($ref['referralDate']); ?></p>
                                    <div class="flex gap-2">
        ...
        <?php if ($tab === 'analytics'): ?>
            <div class="space-y-4 sm:space-y-5">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div class="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-gray-100">
                        <h3 class="font-bold text-gray-900 mb-4">Monthly Trends (Janâ€“Jun 2026)</h3>
                        <div class="space-y-2 text-sm text-gray-600">
        ...
        <div class="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form class="p-5 space-y-4" method="post">
                ...
                <div>
                    ...
                    <input name="diagnosis" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" required>
                </div>
                <div class="mt-2"><label class="block text-sm font-semibold text-gray-700 mb-1">Reason for Referral</label><textarea name="reason" rows="3" required class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"></textarea></div>
